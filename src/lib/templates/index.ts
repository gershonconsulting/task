// Process templates. projectType: simple=single user, advanced=multi-user+dates, complex=multi-fields
export type Priority = 'low' | 'medium' | 'high';
export type ProjectType = 'simple' | 'advanced' | 'complex';

export interface TemplateTask {
  id: string; name: string; description?: string;
  assignedTo: string | null; priority?: Priority;
  dueOffsetDays?: number; tool?: string;
}

export interface ProcessTemplate {
  slug: string; label: string; icon: string; color: string;
  description: string; category?: string; projectType?: ProjectType;
  tasks: TemplateTask[];
}

export interface Tool { slug: string; label: string; icon: string; color: string; url?: string; }

export const DEFAULT_TOOLS: Tool[] = [
  { slug: 'streak', label: 'Streak', icon: '📊', color: '#f97316', url: 'streak.com' },
  { slug: 'chat', label: 'Chat', icon: '💬', color: '#6366f1' },
  { slug: 'calendar', label: 'Calendar', icon: '📅', color: '#0ea5e9' },
  { slug: 'xero', label: 'Xero', icon: '💳', color: '#13b5ea', url: 'xero.com' },
  { slug: 'owlead', label: 'Owlead', icon: '🦉', color: '#7c3aed' },
  { slug: 'linalysis', label: 'Linalysis', icon: '🔎', color: '#059669' },
  { slug: 'cloudcampaign', label: 'CloudCampaign', icon: '📣', color: '#ec4899', url: 'cloudcampaign.com' },
  { slug: 'phantombuster', label: 'PhantomBuster', icon: '👻', color: '#8b5cf6', url: 'phantombuster.com' },
  { slug: 'kular', label: 'Kular', icon: '🎯', color: '#f59e0b', url: 'kular.ai' },
];
...
// SETUP & LIFECYCLE
{
  slug: 'client-onboarding', label: 'Client Onboarding', icon: '🎯', color: '#CC3333',
  description: 'Setup checklist for a new client engagement.', category: 'Setup & Lifecycle', projectType: 'complex',
  tasks: [
...
{
  slug: 'end-of-project', label: 'End of Project', icon: '🏁', color: '#666666',
  description: 'Wind down + final billing on engagement close.', category: 'Setup & Lifecycle', projectType: 'advanced',
  tasks: [
...
// MONTHLY RECURRING
{
  slug: 'monthly-report', label: 'Monthly Report', icon: '📊', color: '#3366CC',
  description: 'End-of-month status report delivery.', category: 'Monthly Recurring', projectType: 'complex',
  tasks: [
...
{
  slug: 'facturation', label: 'Facturation', icon: '💳', color: '#9933CC',
  description: 'Invoicing and payment collection cycle.', category: 'Monthly Recurring', projectType: 'advanced',
  tasks: [
...
{
  slug: 'social-content-creation', label: 'Social Content Creation', icon: '📝', color: '#3366CC',
  description: 'Monthly social content production via CloudCampaign.', category: 'Monthly Recurring', projectType: 'advanced',
  tasks: [
...
{
  slug: 'lead-generation', label: 'Lead Generation', icon: '🎣', color: '#FF6600',
  description: 'Monthly B2B outbound lead gen via PhantomBuster + LinkedIn.', category: 'Monthly Recurring', projectType: 'advanced',
  tasks: [
...
{
  slug: 'social-selling', label: 'Social Selling', icon: '💼', color: '#00AA66',
  description: 'Monthly LinkedIn-based social selling activity.', category: 'Monthly Recurring', projectType: 'advanced',
  tasks: [
...
// ONBOARDING
{
  slug: 'social-content-creation-onboarding', label: 'Social Content Creation — Onboarding', icon: '📝', color: '#1d4ed8',
  description: 'One-time setup for Social Content Creation service.', category: 'Onboarding', projectType: 'complex',
  tasks: [
...
{
  slug: 'lead-generation-onboarding', label: 'Lead Generation — Onboarding', icon: '🎣', color: '#c2410c',
  description: 'One-time setup for Lead Generation service.', category: 'Onboarding', projectType: 'complex',
  tasks: [
...
{
  slug: 'social-selling-onboarding', label: 'Social Selling — Onboarding', icon: '💼', color: '#047857',
  description: 'One-time setup for Social Selling service.', category: 'Onboarding', projectType: 'complex',
  tasks: [
...
// BILLING
{
  slug: 'market-research', label: 'Market Research', icon: '🔍', color: '#CCCC00',
  description: 'Full market research report with 3 versions and exec summary.', category: 'Billing', projectType: 'complex',
  tasks: [
...
// PEOPLE & PARTNERS
{
  slug: 'staff-onboarding', label: 'Staff Onboarding', icon: '🧑‍💼', color: '#66CCCC',
  description: 'New Gershon team member intake.', category: 'People & Partners', projectType: 'advanced',
  tasks: [
...
{
  slug: 'onboarding-partner', label: 'Onboarding Partner', icon: '🤝', color: '#996633',
  description: 'External partner intake (e.g. Straight-in, Kular).', category: 'People & Partners', projectType: 'advanced',
  tasks: [
...
// GRANTS
{
  slug: 'grant-request', label: 'Grant Request', icon: '🏛️', color: '#7C3AED',
  description: 'Full grant application process from research to submission and follow-up.', category: 'Grants', projectType: 'complex',
  tasks: [
    { id: 'gr-identify', name: 'Identify eligible grant programs', assignedTo: 'client', priority: 'high', dueOffsetDays: 0 },
    { id: 'gr-eligibility', name: 'Review eligibility criteria', assignedTo: 'Aina Rama', priority: 'high', dueOffsetDays: 2 },
    { id: 'gr-research', name: 'Research funder priorities & past awards', assignedTo: 'Aina Rama', priority: 'high', dueOffsetDays: 3 },
    { id: 'gr-docs', name: 'Gather required documents (financials, registration)', assignedTo: 'client', priority: 'high', dueOffsetDays: 5 },
    { id: 'gr-narrative', name: 'Draft project narrative / statement of need', assignedTo: 'Winnie Lauren', priority: 'high', dueOffsetDays: 7 },
    { id: 'gr-budget', name: 'Prepare detailed budget', assignedTo: 'Olivier', priority: 'high', dueOffsetDays: 8, tool: 'xero' },
    { id: 'gr-timeline', name: 'Create project timeline & milestones', assignedTo: 'Aina Rama', priority: 'medium', dueOffsetDays: 9 },
    { id: 'gr-letters', name: 'Collect letters of support / references', assignedTo: 'client', priority: 'medium', dueOffsetDays: 10 },
    { id: 'gr-review-internal', name: 'Internal review of full application', assignedTo: 'Olivier', priority: 'high', dueOffsetDays: 12 },
    { id: 'gr-client-review', name: 'Client review & approval', assignedTo: 'client', priority: 'high', dueOffsetDays: 14, tool: 'chat' },
    { id: 'gr-revisions', name: 'Revise based on feedback', assignedTo: 'Winnie Lauren', priority: 'medium', dueOffsetDays: 16 },
    { id: 'gr-format', name: 'Format application per funder guidelines', assignedTo: 'Winnie Lauren', priority: 'medium', dueOffsetDays: 17 },
    { id: 'gr-qc', name: 'Final quality check & proofreading', assignedTo: 'Aina Rama', priority: 'high', dueOffsetDays: 18 },
    { id: 'gr-submit', name: 'Submit application via portal/mail', assignedTo: 'Winnie Lauren', priority: 'high', dueOffsetDays: 19 },
    { id: 'gr-confirm', name: 'Confirm submission receipt', assignedTo: 'Winnie Lauren', priority: 'medium', dueOffsetDays: 20, tool: 'chat' },
    { id: 'gr-followup', name: 'Follow up with funder (if no reply in 30 days)', assignedTo: 'Aina Rama', priority: 'medium', dueOffsetDays: 50, tool: 'chat' },
    { id: 'gr-result', name: 'Record grant result (awarded/declined)', assignedTo: 'Olivier', priority: 'medium', dueOffsetDays: 90 },
    { id: 'gr-reporting', name: 'Prepare grant reporting schedule (if awarded)', assignedTo: 'Aina Rama', priority: 'medium', dueOffsetDays: 95 },
  ],
},
];
...
export const PERSONAL_TEMPLATES: ProcessTemplate[] = [
  { slug: 'personal-move', label: 'Move', icon: '📦', color: '#7C3AED', description: 'Personal relocation checklist.', projectType: 'simple', tasks: [
  ...
  { slug: 'personal-trip', label: 'Trip', icon: '✈️', color: '#0EA5E9', description: 'Travel planning checklist.', projectType: 'simple', tasks: [
  ...
  { slug: 'personal-generic', label: 'Personal project', icon: '🎯', color: '#10B981', description: 'Generic personal project.', projectType: 'simple', tasks: [
...
export const MONTHLY_TEMPLATE_SLUGS = [
  'monthly-report', 'facturation', 'social-content-creation', 'lead-generation', 'social-selling',
];

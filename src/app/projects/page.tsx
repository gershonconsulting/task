                ...
                <div className="max-w-6xl mx-auto px-6 py-8">
                    ...
                    {dbError && (
                        <div className="mb-6 p-4 rounded-md bg-red-50 border border-red-200 text-red-800 text-sm">
                                    <strong>Database error:</strong>strong> {dbError}
                        </div>div>
                        )}
                
                    {!dbError && rows && rows.length === 0 && (
                        <div className="text-center py-16 bg-white rounded-lg border border-dashed border-slate-300">
                                    <p className="text-slate-500 mb-4">No projects yet.</p>p>
                    ...
                    {!dbError && rows && rows.length > 0 && (
                        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {rows.map((r) => <ProjectCard key={r.project_id} row={r} />)}
                        </ul>ul>
                        )}
                </div>div>
          </div>div>
                                ...
                                <Link href={`/projects/${row.project_id}`} className="block">
                                        <div className="flex items-start justify-between gap-2 mb-3">
                                                  <div>
                                                              ...
                                                              <h2 className="text-lg font-semibold text-slate-900 mt-1 truncate">{row.company_name}</h2>h2>
                                                  </div>div>
                                                  <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full"
                                                                  style={{
                                                                                    backgroundColor: row.status === 'completed' ? '#d1fae5' : row.status === 'in_progress' ? '#dbeafe' : row.status === 'archived' ? '#e5e7eb' : '#fef3c7',
                                                                                    color: row.status === 'completed' ? '#065f46' : row.status === 'in_progress' ? '#1e40af' : row.status === 'archived' ? '#374151' : '#92400e',
                                                                  }}>
                                                      {row.status.replace('_', ' ')}
                                                  </span>span>
                                        </div>div>
                                        <div className="text-xs text-slate-500 mb-1.5">{row.completed_tasks} / {row.total_tasks} tasks done</div>div>
                                        <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                                                  <div className="h-2 rounded-full transition-all" style={{ width: `${row.percent_complete}%`, backgroundColor: accent }} />
                                        </div>div>
                                        <div className="flex items-center justify-between mt-3 text-xs">
                                                  <span className="font-semibold" style={{ color: accent }}>{row.percent_complete}%</span>span>
                                                  <span className="text-slate-400">Updated {new Date(row.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>span>
                                        </div>div>
                                </Link>Link>
                          </li>li>
                        )
